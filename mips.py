...
def args_to_dict(args):
    ...
    options = {'type' : args[0], 'filename': args[1], 'display': False, 'memory':1000, 'start':0, 'debug':False}
    ind = 2
    while ind < args_len:
        if args[ind] == '-d':
...
def main(args):
    try:
        ...
        print(e)
        print('mips.py twoints|array|noinp <filename> [-d] [-m <# words>] [-s <start addr>] [-D]')
        return

    machine = MipsMachine(options['memory'])
    machine.run(options['type'], options['filename'], options['start'], options['debug'])
